...
$(function() {
    ...
    $('#dateRangePicker').daterangepicker({
        ...
        autoUpdateInput: false, // 사용자가 입력 필드에 날짜를 수동으로 입력할 수 있도록 설정
		startDate: startDateParam ? moment(startDateParam) : moment(), // URL에서 가져온 시작 날짜
        endDate: endDateParam ? moment(endDateParam) : moment().add(1, 'days'), // URL에서 가져온 종료 날짜
    }, function(start, end) {
        // 선택된 날짜가 변경될 때 입력 필드에 날짜를 설정
        $('#dateRangePicker').val(start.format('YYYY-MM-DD') + ' - ' + end.format('YYYY-MM-DD'));
    });
	
	// URL에서 날짜가 있을 경우 입력 필드에 설정
	   if (startDateParam && endDateParam) {
	       $('#dateRangePicker').val(startDateParam + ' - ' + endDateParam); // 날짜 입력 필드에 날짜 설정
	   }
});

// 페이지 로드 시 서치바에 키워드 및 강아지 타입 설정
$(document).ready(function() {
    const urlParams = new URLSearchParams(window.location.search);
    const keyword = urlParams.get('keyword');
    const dogWeight1 = urlParams.get('dogWeight1') || 0; // S 마리수
    const dogWeight2 = urlParams.get('dogWeight2') || 0; // M 마리수
    const dogWeight3 = urlParams.get('dogWeight3') || 0; // L 마리수

    if (keyword) {
        document.getElementById('searchInput').value = keyword; // 서치바에 키워드 설정
    }
    
    // 강아지 수량 설정
    document.getElementById('dogWeight1').value = dogWeight1; // S 마리수 설정
    document.getElementById('dogWeight2').value = dogWeight2; // M 마리수 설정
    document.getElementById('dogWeight3').value = dogWeight3; // L 마리수 설정

    // 강아지 선택 적용
    applyDogSelection(); // 강아지 선택 버튼 텍스트 업데이트
});

// 검색 리다이렉트 함수
function redirectToSearch(page = 0) {
	...
	const filter = document.getElementById('filterDropdown').value; // 선택된 필터 값
	...
	// 검색 URL 형식에 맞춰서 리다이렉트
    let url = `http://localhost:8080/hotelList?page=${page}&keyword=${encodeURIComponent(keyword)}&startDate=${startDate}&endDate=${endDate}`;
